...
- 用户已确认：SQLite 取代 PostgreSQL，成为默认且唯一的官方主线存储
- 默认数据库路径：`<project>/.velaris-agent/velaris.db`（由 `cwd` 推导）
- 旧的 JSON session 文件（`latest.json / session-*.json`）已彻底停用：不再读写、也不做迁移
...
4. 当前代码事实：
   - `src/velaris_agent/persistence/factory.py` 已收束为 SQLite 工厂，并幂等 bootstrap schema
   - `src/openharness/services/session_storage.py` 已切到 SQLite session_records，不再读写旧 JSON 文件
   - `src/velaris_agent/velaris/orchestrator.py` 已采用 envelope-first 输出，并在 barrier 阶段写入 session/execution
   - `src/openharness/tools/biz_execute_tool.py` 已收束为“标准化 request -> 标准化 envelope”薄桥接

## 4. NFR 总览

| 编号 | 维度 | 本单元正式要求 |
| --- | --- | --- |
| NFR-UOW1-01 | 可靠性 / 持久化真相源 | `execution / task / outcome / audit / session` 统一以 SQLite 为主存储，默认语义不允许悄悄回退到旧 JSON / 文件 / 内存真相源 |
| NFR-UOW1-02 | 可用性 / 故障语义 | SQLite 不可用（目录不可写/损坏/初始化失败）时必须 fail-closed，禁止进入真实 scenario 执行 |
| NFR-UOW1-03 | 审计一致性 / 治理 | 高风险必须具备可追溯审计语义；中风险允许 degraded 继续，但补审计必须可追踪；低风险按普通策略 |
| NFR-UOW1-04 | 性能 | `request intake -> planning -> routing -> authority -> effective risk -> gate` 的额外开销 `p95 < 500ms` |
| NFR-UOW1-05 | 安全 / 数据保护 | payload、audit、envelope 均须脱敏 / 最小暴露；SQLite 文件路径与权限边界必须明确且可验证 |
...
1. 本单元范围内的 `execution / task / outcome / audit / session` 必须统一以 SQLite 作为正式主存储。
2. 旧 JSON session 文件与其它文件型回退实现，不得再作为默认业务真相源。
3. 同一 execution 的状态推进、审计引用与恢复锚点，必须能在 SQLite 中得到可回放、可核验的持久化结果。
4. `session` 与 `execution` 虽然语义分离，但都必须落在同一 SQLite 主线上，避免多真相源导致恢复边界模糊。

#### 验收边界

- 默认链路中，不允许“SQLite 不可用但自动落到旧 JSON / 文件 / 内存后继续对外宣称成功”的行为
- 恢复 execution 时，不需要依赖文件系统上的旧 JSON 才能识别 execution 是否存在
- 后续单元若新增持久化对象，应延续 SQLite 单库主线，而不是重新引入并行真相源

### NFR-UOW1-02 可用性与故障策略

#### 要求

1. SQLite 不可用、写入失败或 schema 初始化失败时，`UOW-1` 默认策略为 **fail-closed**。
2. fail-closed 必须发生在真实 scenario 执行之前；不能先执行真实业务，再在返回时才发现持久化失败。
3. 基础设施不可用必须与业务 gate deny 区分表达：
   - infrastructure unavailable = 基础设施故障（SQLite 不可用/不可写/损坏）
   - denied = 治理阻断（业务语义）
...
- SQLite 不可用时，不产生“completed / partially_completed / degraded 成功结果”假象
...
1. 高风险 execution 默认继承 Functional Design 中的 fail-closed 语义，并增加“审计链路不可用时不得假装成功”的约束。
2. 中风险 execution 允许以 `degraded_mode = true` 继续，但补审计不得静默丢失，必须至少满足：
   - 有可追踪的补写任务、状态或错误标记
   - 最终审计仍以 SQLite 为落点（同库）
...
4. 若因为同步审计导致性能超线，必须优先保证语义正确与审计正确，再通过后续设计优化，而不是牺牲正确性。
...
3. SQLite 主线必须满足以下最低安全基线：
   - 数据落库前默认脱敏（复用 `PayloadRedactor`）
   - 数据库文件路径边界明确：仅允许写入 `<project>/.velaris-agent/`
   - 禁止写入敏感目录（例如 `.ssh`、`/etc` 等）
   - 文件权限遵循最小权限原则（结合宿主机权限模型）
...
2. `biz_execute` 输出允许把大部分旧字段迁入新的 envelope 结构，只保留极少数兼容别名字段。
3. 本单元建议保留的过渡别名字段仅限：
   - `session_id`
   - `result`
   - `outcome`
   - `audit_event_count`
4. 桥接层兼容应通过过渡 alias 完成，而不是继续让 `OpenHarness` 承担内部 contract 的解释责任。
...
当前代码已完成“SQLite 主线 + envelope-first + 最小桥接”的关键收束，但仍存在一些后续单元需要继续固化的风险点：

1. execution resume contract 仍未形成独立入口（需要以 `execution_id` 为中心补齐恢复/修复路径）。
2. 中风险补审计虽然有 `audit_status` 语义，但补写调度（同步/异步/修复命令）尚未形成明确闭环。
3. scenario profile 风险判定目前仍为编排层内联映射；后续场景扩张后建议抽离为独立 assessor 组件。
...
- 语义主语义收束到 `Velaris`
- 持久化主线收束到 SQLite 单库
- SQLite 不可用时默认 fail-closed
- 高/中/低风险治理与审计语义必须可区分且可追踪
- 对外 contract 以 envelope 为准，仅保留极少数兼容 alias
- 安全基线必须同时覆盖 payload 脱敏与 SQLite 文件路径/权限边界
