...
**Layer 1: Goal-to-Plan Compiler** — `build_capability_plan()` 把自然语言目标编译为结构化 plan（场景、能力集、权重、治理约束、推荐工具链）。

**Layer 2: Routing & Governance** — `PolicyRouter` 基于风险等级、任务复杂度、副作用范围选择执行策略（local / delegated / hybrid）；`AuthorityService` 签发短时能力令牌。

**Layer 3: Decision Execution** — Agent 通过工具自主抓取决策所需数据（机票价格、模型成本、车辆状态等），然后执行多维评分和场景逻辑。数据不需要预置，Agent 自己获取。

**Layer 4: Runtime Control** — `TaskLedger` 跟踪执行状态，`OutcomeStore` 回写结果，决策环境快照记录完整决策上下文。

**Learning Loop** — 贯穿全层：`DecisionMemory` 存储完整决策记录，`PreferenceLearner` 从用户实际选择中学习个性化权重（贝叶斯先验 + 指数衰减），`self_evolution_review` 定期复盘。
...
```
用户目标 → Agent 自主抓取数据 → 决策环境快照 → 偏好学习 → 权重更新 → 推荐更准
```

每次决策的完整环境（意图 + 抓取到的数据 + 评分 + 路由 trace + 用户选择 + 满意度）都被快照记录。PreferenceLearner 从实际选择中学习个性化权重，用的人越多，推荐越准。
...
| 能力 | 状态 | 实现 |
|------|------|------|
| Goal → Plan 编译 | ✅ | `build_capability_plan()` 场景识别 + 能力规划 |
| 策略路由 | ✅ | `PolicyRouter` + YAML 规则引擎 |
| 能力签发 | ✅ | `AuthorityService` 短时令牌 |
| 任务跟踪 | ✅ | `TaskLedger` 状态机 |
| Outcome 回写 | ✅ | `OutcomeStore` 结果记录 |
| 决策记忆 | ✅ | SQLite 单库（`SqliteDecisionMemory` + `decision_records`） |
| 偏好学习 | ✅ | `PreferenceLearner` 贝叶斯先验 + 指数衰减 |
| 自进化复盘 | ✅ | `self_evolution_review` 定期触发 |
...
| 方向 | 当前状态 | 目标状态 | 优先级 |
|------|----------|----------|--------|
| **持久化层** | SQLite 单库（项目内 `.velaris-agent/velaris.db`），支持跨会话查询与审计 | 补齐索引/迁移与数据导出能力，形成可运维基线 | P1 |
| **决策环境快照完整性** | DecisionRecord 记录了决策变量，但治理变量（routing trace、authority token）未关联落盘 | 统一快照：决策变量 + 治理变量 + 环境变量一体化存储 | P0 |
| **场景插件化注册** | 硬编码 `_SCENARIO_KEYWORDS` 字典，新场景必须改 engine.py | 插件化 ScenarioRegistry，支持外部注册场景 | P1 |
| **动态路由策略** | 静态 YAML 规则，不感知运行时状态 | 支持运行时指标（负载、成本、SLA）动态调整路由权重 | P1 |
| **多租户隔离** | 单用户 session 级 | tenant 级隔离（决策记忆、偏好、路由策略独立） | P1 |
| **快照对比与回放 API** | 快照数据已落盘，但缺少结构化查询和对比接口 | 提供 diff API：同用户不同时间点的决策环境变化可量化 | P2 |
| **Outcome 聚合分析** | 单条记录查询 | 跨场景、跨时间段的 outcome 聚合统计和趋势分析 | P2 |

架构方向已经对了，核心闭环已经跑通。差的是工程化深度——把内存级的治理状态持久化、把硬编码的场景注册插件化、把静态的路由策略动态化。
