...
在个人决策引擎模式下，Policy Plane 采用三轨结构：

- 行为轨（Descriptive）：学习历史行为与偏好，预测“默认会怎么选”。
- 理性轨（Normative）：在多目标约束下计算“应该怎么选”。
- 仲裁层（Policy）：融合两轨结果并输出主决策、替代决策与偏离代价。
...
- `behaviorOutcome`：用户最终是否采纳、偏离程度、执行延迟。
- `rationalOutcome`：理性策略预期收益与实际收益偏差。
- `arbitrationDelta`：仲裁前后分值变化与触发原因。
