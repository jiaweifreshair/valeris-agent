...
TEAMMATE_COMMAND_ENV_VAR = "VELARIS_TEAMMATE_COMMAND"
LEGACY_TEAMMATE_COMMAND_ENV_VAR = "OPENHARNESS_TEAMMATE_COMMAND"
...
def get_teammate_command() -> str:
    ...
    Resolution order:
    1. ``VELARIS_TEAMMATE_COMMAND`` environment variable — allows the
       operator to point at a specific binary or wrapper script.
    2. ``OPENHARNESS_TEAMMATE_COMMAND`` for legacy compatibility.
    3. The ``velaris`` entry-point on PATH.
    4. The legacy ``openharness`` entry-point on PATH.
    5. The current Python interpreter running the module
    ...
    legacy_override = os.environ.get(LEGACY_TEAMMATE_COMMAND_ENV_VAR)
    if legacy_override:
        return legacy_override

    # Check if we are running as an installed package with an entry-point.
    entry_point = shutil.which("velaris")
    if entry_point:
        return entry_point
...
def build_inherited_env_vars() -> dict[str, str]:
    ...
    Always includes ``VELARIS_AGENT_TEAMS=1`` and the legacy flag, plus any provider/proxy
