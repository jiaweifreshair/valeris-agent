...
from velaris_agent.persistence.job_queue import PostgresJobQueue, SqliteJobQueue, run_jobs_once
...
def test_sqlite_job_queue_roundtrip(tmp_path: Path) -> None:
    """SQLite 队列应完成 enqueue / claim / mark_succeeded / get 最小闭环。"""
...
def test_run_jobs_once_processes_self_evolution_jobs_sqlite(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_run_self_evolution_review_job(*, postgres_dsn: str, payload: dict[str, Any]):
        ...
        assert postgres_dsn == str(database_path)
        processed_payloads.append(dict(payload))
        return {"status": "ok"}
